		...
		while not rospy.is_shutdown():
			...
			if self.isReadyToFly:
				...
				if attach == True:
					...
					dis_x = math.sqrt((var_x-des_x)*(var_x-des_x) + (var_y-des_y)*(var_y-des_y)+ (var_z-des_z)*(var_z-des_z))
					print "Going to:",var_x,var_y, self.curr_pose.pose.position.z
					if dis_x<distThreshold:
						pre_z = self.curr_pose.pose.position.z - 0.05*self.curr_pose.pose.position.z
						des_z = pre_z
						print(self.curr_pose.pose.position.z, pre_z, des_z)
						if self.curr_pose.pose.position.z<20 and self.curr_pose.pose.position.x < 84.5 and self.curr_pose.pose.position.x > 83.5 and self.curr_pose.pose.position.y < -53.5 and self.curr_pose.pose.position.y > -54.5:
